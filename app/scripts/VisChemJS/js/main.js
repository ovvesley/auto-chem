...
function drawBenzene(position) {
  ...
  for (var i = 1; i <= numberOfSides; i += 1) {
    ...
    var ring1 = new _atom["default"]("C", 1, 0, {
...
function drawCyclohexane(position) {
  ...
  for (var i = 1; i <= numberOfSides; i += 1) {
    context.lineTo(Xcenter + size * Math.cos(i * 2 * Math.PI / numberOfSides), Ycenter + size * Math.sin(i * 2 * Math.PI / numberOfSides));
    var ring1 = new _atom["default"]("C", 2, 0, {
      x: Xcenter + size * Math.cos(i * 2 * Math.PI / numberOfSides),
      y: Ycenter + size * Math.sin(i * 2 * Math.PI / numberOfSides)
    }); // let atom1 = [ring1, ring2, ring3, ring4, ring5, ring6];
    //
...
function makeAtom(position) {
  ...
  if (bondType === "single") {
    atom = new _atom["default"]("C", 4, 0, position);
  }
  ...
  if (bondType === "ketone") {
    atom = new _atom["default"]("O", 1, 0, position);
  }

  if (bondType === "primaryAmine") {
    atom = new _atom["default"]("N", 1, 0, position);
  }

  if (bondType === "secondaryAmine") {
    atom = new _atom["default"]("N", 2, 0, position);
  }

  if (bondType === "tertiaryAmine") {
    atom = new _atom["default"]("N", 3, 0, position);
  }
  ...
  if (bondType === "bromine") {
    atom = new _atom["default"]("Br", 1, 0, position);
  }

  if (bondType === "chlorine") {
    atom = new _atom["default"]("Cl", 1, 0, position);
  }

  if (bondType === "fluorine") {
    atom = new _atom["default"]("F", 1, 0, position);
  }
  ...
  if (bondType === "imine") {
    atom = new _atom["default"]("NH", 1, 0, position);
  }
...
function makeCarbon(position) {
  var atom = new _atom["default"]("C", 4, 0, position);
  structure.push(atom);
  console.log(structure);
  return atom;
}
...
function dragStop(event) {
  ...
  context.font = "20px Georgia";
  context.fillText("Charge = -".concat(getStructureCharge()), 50, 50); // context.fillText('yes');
